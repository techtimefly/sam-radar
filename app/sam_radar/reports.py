...
function renderProposalWorkspace(id){const host=document.getElementById('proposal-workspace');if(!host)return;const opp=byId.get(String(id));host.innerHTML=opp?proposalWorkspaceHtml(opp):'';host.scrollIntoView({behavior:'smooth',block:'start'});wireProposalWorkspace(host);}
