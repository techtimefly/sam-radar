...
lookupResults?.addEventListener('click',async event=>{const btn=event.target.closest('[data-save-code]');if(!btn)return;try{await postSearchJson('/api/search-reference/save',{kind:btn.dataset.saveCode,code:btn.dataset.code,title:btn.dataset.title,description:btn.dataset.desc,active:true});await loadSearchIntel(lookupQ?.value||'');}catch(err){lookupResults.insertAdjacentHTML('afterbegin',`<p class="muted">${escapeHtml(err.message||'Save failed')}</p>`);}});
...
coachSave?.addEventListener('click',async()=>{if(!coachDraft){coachResults.innerHTML='<p class="muted">Run Search Coach first.</p>';return;}try{await postSearchJson('/api/search-profiles/save',coachDraft);await loadSearchIntel();coachResults.insertAdjacentHTML('afterbegin','<p class="muted">Draft profile saved.</p>');}catch(err){coachResults.insertAdjacentHTML('afterbegin',`<p class="muted">${escapeHtml(err.message||'Save failed')}</p>`);}});
