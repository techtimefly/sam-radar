...
updateTokenState();
tokenButton.addEventListener('click',()=>{tokenPanel.classList.toggle('open');lanePanel?.classList.remove('open');updateTokenState();});
laneButton?.addEventListener('click',()=>{lanePanel.classList.toggle('open');tokenPanel.classList.remove('open');syncLaneControls();});
...
document.addEventListener('click',event=>{if(!lanePanel?.contains(event.target)&&event.target!==laneButton){lanePanel?.classList.remove('open');laneButton?.setAttribute('aria-expanded','false');}if(!tokenPanel.contains(event.target)&&event.target!==tokenButton){tokenPanel.classList.remove('open');}});
...
cards.forEach(card=>{card.querySelector('.workflow-status').value=card.dataset.status||'new';card.querySelector('.workflow-priority').value=card.dataset.priority||'normal';const archive=card.querySelector('.archive-toggle');if(archive){archive.dataset.archived=String(card.dataset.status==='archived');archive.textContent=card.dataset.status==='archived'?'Unarchive':'Archive';}});
