...
def build_report_payload(payload: dict, profile: BusinessProfile, settings: Settings, unseen: list[dict] | None = None) -> dict:
    ...
    return {"summary": summary, "metrics": metrics, "matches": enriched, "proposals": proposals, "errors": payload.get("errors") or []}
...
def row_html(opp: dict) -> str:
    ...
    <article class="opp" id="opp-{notice_id}" data-id="{notice_id}" data-rec="{e(opp.get('recommendation'))}" data-status="{status}" data-priority="{e(opp.get('workflowPriority'))}" data-new="{str(bool(opp.get('isNew'))).lower()}" data-type="{e(opp.get('type'))}" data-followup="{str(bool(opp.get('followUpReasons'))).lower()}" data-search="{e(search_text)}">
      <div class="opp-head"><div><div class="meta"><span class="rank">#{e(opp.get('rank'))}</span><span class="pill rec-{e(str(opp.get('recommendation') or '').lower())}">{e(opp.get('recommendation'))}</span>{new_badge}<span class="pill status-pill status-{status}">{status}</span><span class="pill urgency-{e(str(opp.get('urgency') or '').lower())}">{e(opp.get('urgency'))}</span><span class="pill priority-{e(opp.get('workflowPriority'))}">{e(opp.get('workflowPriority'))}</span>{proposal_badge}</div><h2>{e(opp.get('title'))}</h2></div><div class="head-actions"><button class="card-toggle" data-id="{notice_id}" type="button" aria-expanded="true">{icon_label("columns", "Collapse")}</button><button class="archive-toggle" data-id="{notice_id}" type="button">{icon_label("archive", "Archive")}</button><button class="proposal-create" data-id="{notice_id}" type="button">{icon_label("plus", proposal_label)}</button><button class="open-detail" data-id="{notice_id}" type="button">{icon_label("file", "View")}</button><a class="sam" href="{e(opp.get('url'))}" target="_blank" rel="noopener">{icon_label("external", "Open SAM.gov")}</a></div></div>
...
function proposalDocumentsHtml(opp){const docs=opp.proposalDocuments||[];const evidence=opp.evidenceSnippets||[];return `<div class="document-intake"><h3>Document Intake</h3><p class="muted">Register solicitation links or local files, then parse them into evidence snippets for proposal review.</p><div class="document-intake-form"><label>Source<select class="proposal-doc-source-type"><option value="url">URL</option><option value="local-path">Local path</option></select></label><label>URL or local path<input class="proposal-doc-source" placeholder="https://... or /path/to/file.docx"></label><label>Label<input class="proposal-doc-label" placeholder="PWS, Q&A, Solicitation"></label><button class="proposal-doc-add refresh" type="button">${iconLabel('plus','Add')}</button></div><div class="proposal-docs">${docs.length?docs.map(doc=>`<article class="proposal-doc"><div><b>${escapeHtml(doc.label||doc.filename||'Document')}</b><small>${escapeHtml(proposalDocStatus(doc))} | ${escapeHtml(doc.contentType||'type unknown')} | ${escapeHtml(doc.source||'')}</small>${doc.parseError?`<small>${escapeHtml(doc.parseError)}</small>`:''}</div><button class="proposal-doc-parse" data-document-id="${escapeHtml(doc.id)}" type="button">${iconLabel('file','Parse')}</button><a class="sam" href="${escapeHtml(doc.sourceType==='url'?doc.source:'#')}" target="_blank" rel="noopener">${iconLabel('external','Open')}</a></article>`).join(''):'<p class="muted">No proposal documents registered yet.</p>'}</div><h3>Evidence</h3><div class="evidence-list">${evidence.length?evidence.map(item=>`<article class="evidence-item"><b>${escapeHtml(item.section||'Evidence')}</b><p>${escapeHtml(item.snippet||'')}</p></article>`).join(''):'<p class="muted">Parse documents to create evidence snippets.</p>'}</div><span class="proposal-doc-message status"></span></div>`;}
async function addProposalDocument(id,root,msg){const writeToken=token();if(!writeToken){tokenPanel.classList.add('open');if(msg)msg.textContent='Unlock editing with APP_WRITE_TOKEN';throw new Error('Token required');}const source=root.querySelector('.proposal-doc-source')?.value.trim();if(!source){if(msg)msg.textContent='Enter a URL or local path.';throw new Error('Source required');}const body={noticeId:id,sourceType:root.querySelector('.proposal-doc-source-type')?.value||'url',source,label:root.querySelector('.proposal-doc-label')?.value||''};const res=await fetch('/api/proposal-documents/add',{method:'POST',headers:{'Content-Type':'application/json','X-SAM-RADAR-TOKEN':writeToken},body:JSON.stringify(body)});const data=await res.json();if(!res.ok||!data.ok)throw new Error(data.error||'Document add failed');const opp=byId.get(String(id));if(opp)opp.proposalDocuments=data.documents||[];return data;}
async function parseProposalDocument(id,documentId,msg){const writeToken=token();if(!writeToken){tokenPanel.classList.add('open');if(msg)msg.textContent='Unlock editing with APP_WRITE_TOKEN';throw new Error('Token required');}const res=await fetch('/api/proposal-documents/parse',{method:'POST',headers:{'Content-Type':'application/json','X-SAM-RADAR-TOKEN':writeToken},body:JSON.stringify({documentId})});const data=await res.json();if(!res.ok)throw new Error(data.error||'Document parse failed');const opp=byId.get(String(id));if(opp){const fresh=await fetch('/api/proposal-documents/'+encodeURIComponent(id),{headers:{'Accept':'application/json'}});const loaded=await fresh.json();if(loaded.ok){opp.proposalDocuments=loaded.documents||[];opp.evidenceSnippets=loaded.evidence||[];}}return data;}
function wireDocumentIntake(root,id){root.querySelector('.proposal-doc-add')?.addEventListener('click',async()=>{const msg=root.querySelector('.proposal-doc-message');try{msg.textContent='Adding document...';await addProposalDocument(id,root,msg);msg.textContent='Document added';openDetail(id,modal.dataset.returnView||'list');}catch(err){msg.textContent=err.message||'Document add failed';}});root.querySelectorAll('.proposal-doc-parse').forEach(btn=>btn.addEventListener('click',async()=>{const msg=root.querySelector('.proposal-doc-message');try{msg.textContent='Parsing document...';await parseProposalDocument(id,btn.dataset.documentId,msg);msg.textContent='Document parsed';openDetail(id,modal.dataset.returnView||'list');}catch(err){msg.textContent=err.message||'Document parse failed';}}));}
...
function proposalWorkspaceHtml(opp){const p=opp.proposal||{};if(!p.id)return '';return `<section class="proposal-workspace panel"><div class="panel-head"><h2>${escapeHtml(opp.title||'Proposal Workspace')}</h2><button class="small-ghost proposal-workspace-close" type="button">${iconLabel('close','Close')}</button></div><div class="proposal-meta"><span><b>Role</b>${escapeHtml(p.role||'prime')}</span><span><b>Stage</b>${escapeHtml(p.stageLabel||p.stage||'Intent')}</span><span><b>Owner</b>${escapeHtml(opp.workflowOwner||'Unassigned')}</span><span><b>Due</b>${escapeHtml(opp.dueDisplay||'n/a')}</span></div>${readinessHtml(p)}${proposalStagesHtml(p)}${proposalDocumentsHtml(opp)}<div class="proposal-actions"><button class="proposal-stage-jump" data-id="${escapeHtml(opp.noticeId)}" data-stage="${escapeHtml(proposalAdjacentStage(p,-1))}" ${proposalAdjacentStage(p,-1)?'':'disabled'} type="button">${iconLabel('columns','Previous')}</button><button class="proposal-stage-jump" data-id="${escapeHtml(opp.noticeId)}" data-stage="${escapeHtml(proposalAdjacentStage(p,1))}" ${proposalAdjacentStage(p,1)?'':'disabled'} type="button">${iconLabel('plus','Next')}</button><button class="open-detail" data-id="${escapeHtml(opp.noticeId)}" type="button">${iconLabel('file','Open Opportunity')}</button><span class="proposal-message status"></span></div></section>`;}
function renderProposalWorkspace(id){const host=document.getElementById('proposal-workspace');if(!host)return;const opp=byId.get(String(id));host.innerHTML=opp?proposalWorkspaceHtml(opp):'';host.scrollIntoView({behavior:'smooth',block:'start'});wireProposalWorkspace(host);wireDocumentIntake(host,id);}
...
async function refreshAfterManualTrack(msg){if(msg)msg.textContent='Tracked. Refreshing report...';await refreshReportInPlace();}
