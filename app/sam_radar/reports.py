...
function lookupItemHtml(item){return `<article class="lookup-item"><b>${escapeHtml(item.code)} - ${escapeHtml(item.title||item.label||'')}</b><p>${escapeHtml(item.description||'')}</p><div class="lookup-actions"><button data-save-code="${escapeHtml(item.kind||'')}" data-code="${escapeHtml(item.code)}" data-title="${escapeHtml(item.title||item.label||'')}" data-desc="${escapeHtml(item.description||'')}" type="button">${iconLabel('save','Save')}</button></div></article>`;}
