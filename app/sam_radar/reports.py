...
def row_html(opp: dict) -> str:
    ...
    return f"""
    <article class="opp" data-id="{notice_id}" data-rec="{e(opp.get('recommendation'))}" data-status="{status}" data-new="{str(bool(opp.get('isNew'))).lower()}" data-type="{e(opp.get('type'))}" data-search="{e(search_text)}">
...
def build_html_report(report: dict) -> str:
    ...
    style = """
:root{color-scheme:light;--ink:#18202a;--muted:#667085;--line:#d7dde5;--bg:#f6f8fb;--panel:#fff;--panel-2:#fbfcfe;--header:#0d2235;--header-text:#fff;--header-muted:#c8d7e5;--blue:#1768ac;--green:#167c57;--amber:#a86500;--red:#b42318;--shadow:0 12px 30px rgba(15,35,55,.08)}[data-theme=dark]{color-scheme:dark;--ink:#eef5fb;--muted:#aab8c8;--line:#33495f;--bg:#07131f;--panel:#102235;--panel-2:#0b1a2a;--header:#06101b;--header-text:#f8fbff;--header-muted:#9fb4c9;--blue:#65b7ff;--green:#67d8a4;--amber:#ffc46b;--red:#ff8f85;--shadow:0 18px 42px rgba(0,0,0,.3)}*{box-sizing:border-box}body{margin:0;font-family:ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:var(--ink);background:var(--bg)}header{background:var(--header);color:var(--header-text);padding:28px 32px 22px;border-bottom:5px solid #27a6a6}.wrap{max-width:1180px;margin:0 auto}h1{margin:0;font-size:30px;letter-spacing:0}.subtitle{margin:8px 0 0;color:var(--header-muted)}.summary{display:grid;grid-template-columns:repeat(5,minmax(120px,1fr));gap:12px;margin-top:22px}.stat{background:rgba(255,255,255,.09);border:1px solid rgba(255,255,255,.18);padding:13px 14px;border-radius:8px;min-height:72px}.stat b{display:block;font-size:24px}.stat span{color:var(--header-muted);font-size:13px}main{padding:22px 32px 42px}.brief{display:grid;grid-template-columns:1.5fr 1fr;gap:18px;margin-bottom:18px}.panel,.opp,.empty{background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:18px;box-shadow:var(--shadow)}.panel h2{margin:0 0 8px;font-size:18px}.panel p{margin:7px 0;color:var(--muted);line-height:1.45}.toolbar{position:sticky;top:0;z-index:5;background:color-mix(in srgb,var(--bg) 94%,transparent);backdrop-filter:blur(8px);border-bottom:1px solid var(--line);padding:12px 0;margin-bottom:16px}.tools{display:flex;gap:10px;flex-wrap:wrap;align-items:center}button,input,select{border:1px solid var(--line);background:var(--panel);color:var(--ink);border-radius:6px;padding:9px 11px;font:inherit}button{cursor:pointer}button.active{border-color:var(--blue);background:color-mix(in srgb,var(--blue) 14%,var(--panel));color:var(--blue)}input{min-width:280px;flex:1}.icon-btn{width:40px;min-width:40px;text-align:center;padding:9px 0}.refresh,.token-save{background:#0d2235;color:white;border-color:#0d2235;font-weight:700}.status{color:var(--muted);font-size:13px;min-width:180px}.view{display:none}.view.active{display:block}.opp{border-left:5px solid var(--blue);margin-bottom:14px}.opp-head{display:flex;justify-content:space-between;gap:16px;align-items:start}.meta{display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-bottom:8px}.rank{color:var(--muted);font-weight:700}.pill{display:inline-flex;align-items:center;border:1px solid var(--line);border-radius:999px;padding:3px 8px;font-size:12px;font-weight:700;background:var(--panel-2)}.rec-pursue,.status-pursue{color:var(--green);border-color:#60bd91;background:color-mix(in srgb,var(--green) 14%,var(--panel))}.rec-monitor,.status-monitor{color:var(--amber);border-color:#d89f4d;background:color-mix(in srgb,var(--amber) 16%,var(--panel))}.status-no-bid,.status-archived{color:var(--muted)}.status-teaming,.status-reviewing,.status-submitted{color:var(--blue);border-color:var(--blue);background:color-mix(in srgb,var(--blue) 13%,var(--panel))}.new{color:#0284c7;border-color:#8bd3f7;background:color-mix(in srgb,#38bdf8 16%,var(--panel))}.urgency-high{color:var(--red);border-color:#fda29b;background:color-mix(in srgb,var(--red) 14%,var(--panel))}.urgency-medium{color:var(--amber);border-color:#d89f4d;background:color-mix(in srgb,var(--amber) 16%,var(--panel))}.urgency-low{color:var(--green);border-color:#60bd91;background:color-mix(in srgb,var(--green) 14%,var(--panel))}.opp h2{margin:0;font-size:20px;line-height:1.25;letter-spacing:0}.sam{white-space:nowrap;text-decoration:none;color:white;background:#1768ac;border-radius:6px;padding:9px 12px;font-weight:700}.facts{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin:16px 0}.facts span,.dims span{border:1px solid var(--line);border-radius:6px;padding:9px 10px;color:var(--muted);min-width:0;overflow-wrap:anywhere;background:var(--panel-2)}.facts b,.dims b{display:block;color:var(--ink);font-size:12px;text-transform:uppercase;letter-spacing:.04em;margin-bottom:3px}.analysis{display:grid;grid-template-columns:1fr 1fr;gap:16px}h3{margin:0 0 7px;font-size:14px}p{line-height:1.45}ul{margin:8px 0 0 18px;padding:0;color:var(--muted)}.muted{color:var(--muted)}.workflow{display:grid;grid-template-columns:180px 1fr auto minmax(120px,auto);gap:10px;align-items:end;margin-top:14px;border-top:1px solid var(--line);padding-top:14px}.workflow label{display:grid;gap:4px;color:var(--muted);font-size:13px}.workflow select,.workflow input{width:100%;min-width:0}.save-status{background:#0d2235;color:white;border-color:#0d2235;font-weight:700}.dims{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px;margin-top:14px}.board{display:grid;grid-template-columns:repeat(4,minmax(220px,1fr));gap:12px}.lane{background:var(--panel);border:1px solid var(--line);border-radius:8px;min-height:180px;padding:12px}.lane h2{font-size:14px;text-transform:uppercase;letter-spacing:.04em;margin:0 0 10px;display:flex;justify-content:space-between;color:var(--muted)}.lane-count{font-weight:800;color:var(--ink)}.board-card{display:block;text-decoration:none;color:var(--ink);background:var(--panel-2);border:1px solid var(--line);border-radius:8px;padding:10px;margin-bottom:10px}.board-card b{display:block;font-size:14px;line-height:1.25;margin-bottom:8px}.board-card span{display:inline-flex;margin-right:6px;margin-bottom:4px;color:var(--muted);font-size:12px}.token-popover{position:fixed;right:24px;top:96px;z-index:10;display:none;width:min(420px,calc(100vw - 32px));background:var(--panel);border:1px solid var(--line);border-radius:8px;box-shadow:0 24px 60px rgba(0,0,0,.28);padding:16px}.token-popover.open{display:block}.token-popover h2{margin:0 0 6px;font-size:18px}.token-popover p{margin:0 0 12px;color:var(--muted)}.token-row{display:flex;gap:8px}.token-row input{min-width:0}.token-actions{display:flex;gap:8px;justify-content:flex-end;margin-top:12px}.read-only{font-weight:700;color:var(--amber)}@media(max-width:1040px){.board{grid-template-columns:repeat(2,minmax(220px,1fr))}}@media(max-width:860px){header,main{padding-left:16px;padding-right:16px}.summary{grid-template-columns:repeat(2,1fr)}.brief,.analysis,.board{grid-template-columns:1fr}.facts{grid-template-columns:1fr 1fr}.dims{grid-template-columns:1fr}.workflow{grid-template-columns:1fr}.opp-head{flex-direction:column}input{min-width:100%}.status{min-width:0}.toolbar{position:static}}
"""
    script = """
...
cards.forEach(card=>{const btn=card.querySelector('.save-status');const select=card.querySelector('.workflow-status');if(select&&btn){select.value=btn.dataset.status||'new';}});
function apply(){const q=(input.value||'').toLowerCase().trim();cards.forEach(card=>{const rec=card.dataset.rec||'';const type=card.dataset.type||'';const isNew=card.dataset.new==='true';const text=(card.dataset.search||'').toLowerCase();let ok=filter==='all'||rec===filter||type===filter||card.dataset.status===filter||(filter==='new'&&isNew);if(q) ok=ok&&text.includes(q);card.style.display=ok?'block':'none';});buildBoard();}
...
function buildBoard(){const board=document.getElementById('board');if(!board)return;const statuses=['new','reviewing','pursue','teaming','submitted','monitor','no-bid','archived'];board.innerHTML=statuses.map(status=>{const items=cards.filter(card=>card.dataset.status===status&&card.style.display!=='none');return `<section class="lane"><h2>${labelStatus(status)} <span class="lane-count">${items.length}</span></h2>${items.map(card=>{const title=card.querySelector('h2')?.textContent||'Untitled';const score=[...card.querySelectorAll('.facts span')].find(s=>s.querySelector('b')?.textContent==='Score')?.childNodes[1]?.textContent||'n/a';const due=[...card.querySelectorAll('.facts span')].find(s=>s.querySelector('b')?.textContent==='Due')?.childNodes[1]?.textContent||'n/a';const agency=[...card.querySelectorAll('.facts span')].find(s=>s.querySelector('b')?.textContent==='Agency')?.childNodes[1]?.textContent||'n/a';return `<a class="board-card" href="#" data-jump="${escapeHtml(card.dataset.id)}"><b>${escapeHtml(title)}</b><span>Score ${escapeHtml(score)}</span><span>Due ${escapeHtml(due)}</span><span>${escapeHtml(agency)}</span></a>`;}).join('')||'<p class="muted">No cards</p>'}</section>`;}).join('');board.querySelectorAll('[data-jump]').forEach(link=>link.addEventListener('click',event=>{event.preventDefault();document.querySelector('[data-view="list"]').click();document.querySelector(`.opp[data-id="${CSS.escape(link.dataset.jump)}"]`)?.scrollIntoView({behavior:'smooth',block:'start'});}));}
...
viewButtons.forEach(btn=>btn.addEventListener('click',()=>{viewButtons.forEach(b=>b.classList.remove('active'));btn.classList.add('active');document.querySelectorAll('.view').forEach(v=>v.classList.remove('active'));document.getElementById(btn.dataset.view+'-view').classList.add('active');buildBoard();}));
...
document.querySelectorAll('.save-status').forEach(btn=>btn.addEventListener('click',async()=>{const card=btn.closest('.opp');const msg=card.querySelector('.workflow-message');let writeToken=token();if(!writeToken){tokenPanel.classList.add('open');msg.textContent='Unlock editing with APP_WRITE_TOKEN';return;}btn.disabled=true;msg.textContent='Saving...';try{const body={status:card.querySelector('.workflow-status').value,notes:card.querySelector('.workflow-notes').value};const res=await fetch('/api/status/'+encodeURIComponent(card.dataset.id),{method:'POST',headers:{'Content-Type':'application/json','X-SAM-RADAR-TOKEN':writeToken},body:JSON.stringify(body)});const data=await res.json();if(!res.ok||!data.ok)throw new Error(data.error||'Save failed');msg.textContent='Saved';btn.dataset.status=data.workflow.status;card.dataset.status=data.workflow.status;const pill=card.querySelector('.status-pill');if(pill){pill.textContent=labelStatus(data.workflow.status);pill.className='pill status-pill status-'+data.workflow.status;}buildBoard();}catch(err){msg.textContent=err.message||'Save failed';if((err.message||'').toLowerCase().includes('token')){localStorage.removeItem('samRadarWriteToken');updateTokenState();}}finally{btn.disabled=false;}}));
apply();
"""
    return f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>SAM Radar</title><style>{style}</style></head><body><header><div class="wrap"><h1>SAM Radar - {e(summary['businessName'])}</h1><p class="subtitle">Generated {e(summary['generatedAtDisplay'])} | Window {e(summary['postedFrom'])} to {e(summary['postedTo'])}</p><div class="summary"><div class="stat"><b>{e(summary['totalMatches'])}</b><span>Total matches</span></div><div class="stat"><b>{e(summary['newMatches'])}</b><span>New unseen</span></div><div class="stat"><b>{e(summary['pursueCount'])}</b><span>Pursue</span></div><div class="stat"><b>{e(summary['monitorCount'])}</b><span>Monitor</span></div><div class="stat"><b>{e(summary['sourcesSoughtCount'])}</b><span>Sources sought</span></div></div></div></header><main><div class="wrap"><section class="brief"><div class="panel"><h2>Executive Read</h2><p><b>Best match:</b> {e(summary['bestMatch'])}</p><p><b>Fastest deadline:</b> {e(summary['fastestDeadline'])} {e(summary['fastestDeadlineDisplay'])}</p><p>This report ranks opportunities by configured capability fit, set-aside signal, actionability, and deadline pressure.</p></div><div class="panel"><h2>BD Notes</h2><p>Sources sought are useful for early agency engagement. High-fit solicitations with short runway need same-day attachment review.</p><p>Use the status workflow to track pursue, teaming, monitor, and no-bid decisions.</p></div></section><div class="toolbar"><div class="tools"><button class="active" data-view="list">List</button><button data-view="board">Board</button><button class="active" data-filter="all">All</button><button data-filter="Pursue">Pursue</button><button data-filter="Monitor">Monitor</button><button data-filter="new">New</button><button data-filter="Sources Sought">Sources Sought</button><input id="q" placeholder="Search title, agency, capability area"><button id="theme-button" class="icon-btn" type="button">Dark</button><button id="token-button" type="button">Unlock</button><button id="refresh" class="refresh">Refresh</button><span id="refresh-status" class="status"></span></div></div><section id="token-panel" class="token-popover"><h2>Editing Token</h2><p id="token-state"></p><p>Use your local APP_WRITE_TOKEN here. This is separate from your SAM.gov API key.</p><div class="token-row"><input id="token-input" type="password" autocomplete="off" placeholder="APP_WRITE_TOKEN"><button id="token-save" class="token-save" type="button">Save</button></div><div class="token-actions"><button id="token-clear" type="button">Clear Browser Token</button></div></section><section id="list-view" class="view active"><section id="list">{rows_html}</section></section><section id="board-view" class="view"><div id="board" class="board"></div></section></div></main><script id="report-data" type="application/json">{data_json}</script><script>{script}</script></body></html>"""
