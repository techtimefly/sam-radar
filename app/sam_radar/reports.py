...
def row_html(opp: dict) -> str:
    ...
    <article class="opp" id="opp-{notice_id}" data-id="{notice_id}" data-rec="{e(opp.get('recommendation'))}" data-status="{status}" data-priority="{e(opp.get('workflowPriority'))}" data-new="{str(bool(opp.get('isNew'))).lower()}" data-type="{e(opp.get('type'))}" data-followup="{str(bool(opp.get('followUpReasons'))).lower()}" data-search="{e(search_text)}">
      ...
      <section class="description-preview"><h3>SAM.gov Description</h3><div>{''.join(f'<p>{e(p)}</p>' for p in (opp.get('descriptionParagraphs') or [])[:2]) or '<p class="muted">No fetched description available.</p>'}</div></section>
...
function descriptionHtml(opp){const paragraphs=opp.descriptionParagraphs||[];return paragraphs.length?paragraphs.map(p=>`<p>${escapeHtml(p)}</p>`).join(''):'<p class="muted">No fetched SAM.gov description available.</p>';}
