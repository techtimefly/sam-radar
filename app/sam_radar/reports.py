...
function manualCard(opp){const id=String(opp.noticeId||'');const already=opp.alreadyTracked||byId.has(id);const label=already?'Already tracked':'Track';const disabled=already?'disabled':'';return `<article class="manual-card" data-manual-id="${escapeHtml(id)}"><div class="meta"><span class="pill">Score ${escapeHtml(opp.score||'n/a')}</span><span class="pill">${escapeHtml(opp.type||'n/a')}</span><span class="pill ${already?'status-archived':'status-reviewing'}">${already?'Already tracked':'Not in report'}</span></div><h2>${escapeHtml(opp.title||'Untitled')}</h2><div class="facts"><span><b>Agency</b>${escapeHtml(opp.organization||'n/a')}</span><span><b>Posted</b>${escapeHtml(opp.postedDate||'n/a')}</span><span><b>Due</b>${escapeHtml(opp.responseDeadline||'n/a')}</span><span><b>NAICS</b>${escapeHtml(opp.naicsCode||'n/a')}</span><span><b>PSC</b>${escapeHtml(opp.classificationCode||'n/a')}</span></div><p class="muted">${escapeHtml((opp.reasons||[]).join('; ')||'Matched manual search criteria.')}</p><div class="manual-actions"><button class="manual-add" data-id="${escapeHtml(id)}" type="button" ${disabled}>${label}</button><a class="sam" href="${escapeHtml(opp.url||'#')}" target="_blank" rel="noopener">Open SAM.gov</a><span class="manual-message muted"></span></div></article>`;}
function renderManual(matches){if(!manualResults)return;manualResults.innerHTML=(matches||[]).map(manualCard).join('')||'<section class="empty">No manual search matches found.</section>';manualResults.querySelectorAll('.manual-add').forEach(btn=>btn.addEventListener('click',async()=>{const id=btn.dataset.id;const msg=btn.closest('.manual-card').querySelector('.manual-message');const opp=(manualMatches||[]).find(item=>String(item.noticeId)===String(id));if(!opp)return;const writeToken=token();if(!writeToken){tokenPanel.classList.add('open');msg.textContent='Unlock editing with APP_WRITE_TOKEN';return;}btn.disabled=true;msg.textContent='Tracking...';try{const res=await fetch('/api/manual-add',{method:'POST',headers:{'Content-Type':'application/json','X-SAM-RADAR-TOKEN':writeToken},body:JSON.stringify(opp)});const data=await res.json();if(res.status===409||data.duplicate){msg.textContent='Already tracked';return;}if(!res.ok||!data.ok)throw new Error(data.error||'Track failed');msg.textContent='Tracked in workflow';btn.textContent='Tracked';}catch(err){btn.disabled=false;msg.textContent=err.message||'Track failed';}}));}
let manualMatches=[];
