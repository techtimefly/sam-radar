...
def build_report_payload(payload: dict, profile: BusinessProfile, settings: Settings, unseen: list[dict] | None = None) -> dict:
    ...
    proposals = [{**(m.get("proposal") or {}), "opportunityTitle": m.get("title"), "agency": m.get("organization"), "dueDisplay": m.get("dueDisplay")} for m in enriched if m.get("proposal")]
...
modal.addEventListener('click',async event=>{const createBtn=event.target.closest('.create-proposal-role');const saveBtn=event.target.closest('.save-proposal-stage');if(!createBtn&&!saveBtn)return;const id=modal.dataset.id;const msg=modal.querySelector('.proposal-message');try{if(createBtn){msg.textContent='Creating proposal...';await createProposal(id,createBtn.dataset.role,msg);msg.textContent='Proposal created';}else{msg.textContent='Saving stage...';await saveProposalStage(id,modal.querySelector('.proposal-stage-select').value,msg);msg.textContent='Stage saved';}openDetail(id,modal.dataset.returnView||'list');}catch(err){msg.textContent=err.message||'Proposal action failed';}});
